...
class AntDirEnv(AntEnv):
    ...
    def __init__(self, max_episode_steps=200):
        ...
        self.task_dim = 1
        self.step_count = 0
        self.eval_task_list = [0.0, 0.5 * np.pi, np.pi, 1.5 * np.pi, 0.25 * np.pi, 0.75 * np.pi, 1.25 * np.pi, 1.75 * np.pi]  # this list is for visualisation
    ...
    def sample_tasks(self, n_tasks):
        oracle = False
        if oracle:
            return [random.choice([0.0, 0.5 * np.pi, np.pi, 1.5 * np.pi, 0.25 * np.pi, 0.75 * np.pi, 1.25 * np.pi, 1.75 * np.pi]) for _ in range(n_tasks, )]
        else:
            return [random.choice([0.0, 0.5*np.pi, np.pi, 1.5*np.pi]) for _ in range(n_tasks, )]
